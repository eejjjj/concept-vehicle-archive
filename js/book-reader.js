...
export class BookReader {
  ...
  _progressFromPointer(layoutX, layoutY, drag) {
    const L = this._screenLayout();
  ...
  _onPointerDown(e) {
    ...
    applyCurl(this._flipGeo, 0, PAGE_W, hit.pivotY);
    this._updateFlipShadow(0, hit.dir, hit.pivotY);
  }

  _onPointerMove(e) {
    if (!this._drag || this.flipping || !this._drag.ready) return;
    const p = clientToCanvasLayout(this.canvas, e.clientX, e.clientY);
    this._drag.lastX = p.x;
    this._drag.lastY = p.y;
    const { progress, pivotY } = this._progressFromPointer(p.x, p.y, this._drag);
