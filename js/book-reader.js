...
/**
 * Matte silver ink (哑光银油墨): mid-greys → neutral metallic field;
 * blacks stay ink; near-whites (badge type) stay bright.
 */
function patchSilverMaterial(mat) {
  mat.toneMapped = false;
  mat.customProgramCacheKey = () => 'emax-silver-ink-v2';
  mat.onBeforeCompile = (shader) => {
    /* Neutral silver — no cool/blue cast */
    shader.uniforms.uSilverLo = { value: new THREE.Color(0x929292) };
    shader.uniforms.uSilverHi = { value: new THREE.Color(0xc8c8c8) };
    shader.uniforms.uSilverInk = { value: new THREE.Color(0x121212) };
    shader.uniforms.uSilverWhite = { value: new THREE.Color(0xeaeaea) };
    shader.fragmentShader = shader.fragmentShader
      .replace(
        '#include <common>',
        /* glsl */ `
#include <common>
uniform vec3 uSilverLo;
uniform vec3 uSilverHi;
uniform vec3 uSilverInk;
uniform vec3 uSilverWhite;
`,
      )
      .replace(
        '#include <map_fragment>',
	...
	/* Separate ink / silver field / highlight type */
	float inkW = 1.0 - smoothstep( 0.04, 0.22, lum );
	float whiteW = smoothstep( 0.72, 0.92, lum );
	float metalW = clamp( 1.0 - inkW - whiteW, 0.0, 1.0 );

	/* Fine pigment grain + soft brushed anisotropy */
	float g1 = fract( sin( dot( vMapUv * vec2( 720.0, 720.0 ), vec2( 12.9898, 78.233 ) ) ) * 43758.5453 );
	float g2 = fract( sin( dot( vMapUv * vec2( 1400.0, 90.0 ), vec2( 39.346, 11.135 ) ) ) * 23421.631 );
	float brush = mix( g1, g2, 0.55 );
	/* Broad matte foil falloff — not chrome */
	float foil = 0.5 + 0.5 * cos( ( vMapUv.x * 2.15 + vMapUv.y * 0.55 ) * 3.14159265 );
	float sheen = mix( 0.88, 1.1, foil * 0.55 + brush * 0.45 );
	vec3 silver = mix( uSilverLo, uSilverHi, brush * 0.42 + foil * 0.28 );
	silver *= sheen;
	/* Micro specular flecks in the ink film */
	silver += vec3( 0.035 ) * smoothstep( 0.82, 0.98, brush ) * foil;

	vec3 printRgb = uSilverInk * inkW + uSilverWhite * whiteW + silver * metalW;
	diffuseColor = vec4( printRgb, diffuseColor.a * sampledDiffuseColor.a );
#endif
`,
      );
...
function pageMat(tex, { silver = false } = {}) {
  ...
  return silver ? patchSilverMaterial(mat) : patchPaperMaterial(mat);
}
...
/** Dark paper-edge strip from inverted page.png — matches book block #2e2d27 */
function makePaperEdgeTexture(pageTex) {
  ...
  if (img) {
    ...
    /* After 90° CW, draw into swapped extents */
    ctx.drawImage(tmp, -h / 2, -w / 2, h, w);
    ctx.restore();
  }
...
function makeMatteSilverTexture() {
  ...
  c.height = s;
  const ctx = c.getContext('2d');
  /* Pale silver, one step deeper */
  const g = ctx.createLinearGradient(0, 0, s, s);
  g.addColorStop(0, '#cdd2d9');
  g.addColorStop(0.28, '#dce1e8');
  g.addColorStop(0.48, '#e8ebef');
  g.addColorStop(0.62, '#dbe0e6');
  g.addColorStop(1, '#c6ccd4');
  ctx.fillStyle = g;
  ctx.fillRect(0, 0, s, s);
  /* Brushed striations */
  for (let y = 0; y < s; y += 1) {
    const a = 0.025 + ((y * 13) % 7) * 0.004;
    const v = 210 + ((y * 17) % 40);
    ctx.fillStyle = `rgba(${v}, ${v + 2}, ${v + 5}, ${a})`;
  ...
  for (let i = 0; i < 1800; i += 1) {
    ...
    const y = (i * 91) % s;
    const v = 220 + (i % 28);
    ctx.fillStyle = `rgba(${v}, ${v + 1}, ${v + 3}, 0.04)`;
...
function makeCaseSpineTexture() {
  ...
  g.addColorStop(0, '#bdc2c9');
  g.addColorStop(0.18, '#ccd1d8');
  g.addColorStop(0.5, '#d8dce2');
  g.addColorStop(0.82, '#ccd1d8');
  g.addColorStop(1, '#bdc2c9');
  ctx.fillStyle = g;
  ctx.fillRect(0, 0, w, h);
...
export class BookReader {
  ...
  _buildScene() {
    const g = this.group;
    g.add(new THREE.AmbientLight(0xffffff, 0.42));
    /* Cool key/fill + rim so the open case picks up silver highlights */
    const key = new THREE.DirectionalLight(0xf4f7fb, 0.58);
    key.position.set(0.55, 0.85, 1.4);
    g.add(key);
    const fill = new THREE.DirectionalLight(0xd8dee6, 0.38);
    fill.position.set(-0.7, -0.15, 0.9);
    g.add(fill);
    const rim = new THREE.DirectionalLight(0xe8eef6, 0.28);
    rim.position.set(-0.2, 0.4, -1.1);
    g.add(rim);
  ...
  async _buildCase() {
    ...
    const silverMap = makeMatteSilverTexture();
    ...
    rightBoard.material.map = silverMap;
  ...
  _matForPage(page1, tex, { opacity = null, mirror = false } = {}) {
    ...
    const key = `${page1}|${mirror ? 1 : 0}|${opacity == null ? 'solid' : opacity.toFixed(3)}|${isSilverPage(page1) ? 'ag' : 'p'}`;
    const cached = this._pageMatCache.get(key);
    if (cached) return cached;

    let mat;
    if (opacity != null) mat = vellumMat(face, opacity);
    else if (isVellumPage(page1)) mat = vellumMat(face);
    else mat = pageMat(face, { silver: isSilverPage(page1) });
  ...
  _buildStackMeshesSync(stack, xCenter, opts = {}) {
    ...
    for (let i = n - 1; i >= 0; i -= 1) {
      ...
      const mat = !srcTex
        ? paperMat(isBase ? 0xf2eee6 : 0xe8e4dc)
  ...
  /** Mobile: either page is a valid swipe start; direction comes from finger travel */
  ...
  _updateFlipShadow(progress, dir) {
  ...
  _setFlipMaterials(frontTex, backTex, mirrored, {
    ...
    this._flipFront.material = frontMap
      ? frontVellum
        ...
        : pageMat(frontMap, { silver: frontSilver })
      : paperMat();
  ...
  /** Finger progress → curl + shadow + stack (edge speed stays even with the finger). */
  _applyFlipVisual(fingerProgress, dir) {
    ...
    this._updateFlipShadow(curl, dir);
  ...
  _onPointerMove(e) {
    if (!this._drag || this.flipping) return;
