...
import { SPREAD_COUNT, spreadPages } from './book-pages.js?v=p13';
...
const ASSETS = {
  ...
  /** Closed hardcover — keep GLB materials; lenticular overlays the cover square */
  bookModel: ap('assets/new_book_v02.glb'),
  bookModelFabric: ap('assets/new_book_fabric.glb'),
};
...
const BookViewer = (() => {
  ...
  /** Darken case metal only — keep exported metalness / roughness */
  const SHELL_ALBEDO = 0.68;
  const SHELL_ENV = 0.36;
  ...
  /**
   * new_book_v02: Y = thickness (cover outer on +Y), cover in XZ.
   * Site: Z+ cover, Y+ up. Scale face to COVER_W.
   * Then yaw the shell 90° clockwise (front view) so the spine sits correctly;
   * the lenticular overlay is added later unrotated, so the sheet stays upright.
   */
  function fitArchivalBookModel(sceneRoot) {
    ...
    /* Clockwise as seen looking at the cover (+Z). */
  ...
  function holdRingMat() {
    const col = bookVolume() === 'next'
      ? new THREE.Color(0x3466ff)
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        /* Clockwise from top-left along a square frame. */
        float squareClockwise01(vec2 p) {
          float ax = abs(p.x);
          float ay = abs(p.y);
          float m = max(ax, ay);
          vec2 n = p / max(m, 1e-5);
          if (ay >= ax) {
            if (p.y >= 0.0) {
              return (n.x + 1.0) * 0.125;
            }
            return 0.5 + (1.0 - n.x) * 0.125;
          }
          if (p.x >= 0.0) {
            return 0.25 + (1.0 - n.y) * 0.125;
          }
          return 0.75 + (n.y + 1.0) * 0.125;
        }

        void main() {
          vec2 p = vUv * 2.0 - 1.0;
          /* Groove sits just outside the lenticular, inside this slightly larger plane. */
          vec2 box = vec2(0.905);
          float rad = 0.006;
          float d = abs(sdRoundedBox(p, box, rad));
          float core = smoothstep(0.026, 0.005, d);
          float halo = smoothstep(0.058, 0.010, d);
          float t = squareClockwise01(p);
          float filled = 1.0 - smoothstep(uProgress, uProgress + 0.02, t);
          if (uProgress >= 0.999) filled = 1.0;
          float head = uProgress > 0.004
            ? smoothstep(0.04, 0.0, abs(fract(t - uProgress + 0.5) - 0.5))
            : 0.0;
          float track = halo * 0.24 + core * 0.2;
          float glow = (halo * 0.55 + core) * filled + core * head * 1.15;
          vec3 col = uColor;
          float a = (track + glow) * uOpacity;
          if (a < 0.004) discard;
  ...
  function addHoldRing(lenticular) {
    const { width: w, height: h } = lenticular.geometry.parameters;
    /* Cover the square recess around the plate, not a circle outside it. */
    const mesh = new THREE.Mesh(new THREE.PlaneGeometry(1, 1), holdRingMat());
    mesh.scale.set(w * 1.085, h * 1.085, 1);
  ...
  function isCoverStripeMesh(obj) {
    if (!obj?.isMesh) return false;
    return meshMaterials(obj).some((m) => /条纹/.test(m?.name || ''));
  }

  /** Front-cover well after fit: floor + inner lip of the recess. */
  function measureFrontCoverWell(coverMesh) {
    coverMesh.updateMatrixWorld(true);
    const geom = coverMesh.geometry;
    const pos = geom.attributes.position;
    const nrm = geom.attributes.normal;
    const idx = geom.index;
    if (!pos || !nrm) return null;
    const mw = coverMesh.matrixWorld;
    const nm = new THREE.Matrix3().getNormalMatrix(mw);
    const zOuter = new THREE.Box3().setFromObject(coverMesh).max.z;
    ...
    let floorHits = 0;
    let lipMinX = Infinity;
    let lipMaxX = -Infinity;
    let lipMinY = Infinity;
    let lipMaxY = -Infinity;
    let wallHits = 0;
    const inWellRing = (x, y) => {
      const ax = Math.abs(x);
      const ay = Math.abs(y);
      return (ax > 0.18 && ax < 0.28 && ay < 0.28) || (ay > 0.18 && ay < 0.28 && ax < 0.28);
    };
    ...
    for (let t = 0; t < triCount; t += 1) {
      ...
      nWorld.fromBufferAttribute(nrm, i0).applyMatrix3(nm).normalize();
      if (
        nWorld.z > 0.7 &&
        centroid.z < zOuter - 0.0035 &&
        centroid.z > zOuter - 0.012 &&
        Math.abs(centroid.x) < 0.4 &&
        Math.abs(centroid.y) < 0.4
      ) {
        floorMinX = Math.min(floorMinX, a.x, b.x, c.x);
        floorMaxX = Math.max(floorMaxX, a.x, b.x, c.x);
        floorMinY = Math.min(floorMinY, a.y, b.y, c.y);
        floorMaxY = Math.max(floorMaxY, a.y, b.y, c.y);
        floorMaxZ = Math.max(floorMaxZ, a.z, b.z, c.z);
        floorHits += 1;
      }
      if (
        centroid.z > zOuter - 0.012 &&
        centroid.z < zOuter + 0.002 &&
        inWellRing(centroid.x, centroid.y)
      ) {
        if (nWorld.x > 0.55 && centroid.x < 0) {
          lipMinX = Math.min(lipMinX, a.x, b.x, c.x);
          wallHits += 1;
        } else if (nWorld.x < -0.55 && centroid.x > 0) {
          lipMaxX = Math.max(lipMaxX, a.x, b.x, c.x);
          wallHits += 1;
        } else if (nWorld.y > 0.55 && centroid.y < 0) {
          lipMinY = Math.min(lipMinY, a.y, b.y, c.y);
          wallHits += 1;
        } else if (nWorld.y < -0.55 && centroid.y > 0) {
          lipMaxY = Math.max(lipMaxY, a.y, b.y, c.y);
          wallHits += 1;
        }
      }
    }
    if (floorHits < 8 || floorMaxX - floorMinX < 0.05 || floorMaxY - floorMinY < 0.05) return null;
    const hasLip =
      wallHits >= 8 &&
      Number.isFinite(lipMinX) &&
      Number.isFinite(lipMaxX) &&
      Number.isFinite(lipMinY) &&
      Number.isFinite(lipMaxY) &&
      lipMaxX - lipMinX > 0.05 &&
      lipMaxY - lipMinY > 0.05;
    return {
      minX: hasLip ? lipMinX : floorMinX,
      maxX: hasLip ? lipMaxX : floorMaxX,
      minY: hasLip ? lipMinY : floorMinY,
      maxY: hasLip ? lipMaxY : floorMaxY,
      maxZ: Number.isFinite(floorMaxZ) ? floorMaxZ : zOuter,
      zOuter,
    };
  }

  function bookVolume() {
    return document.body?.dataset?.slug === 'next' ? 'next' : 'emax';
  }

  function isCoverMat(m) {
    return /白色封面/.test(m?.name || '');
  }

  function isSpineClothMat(m) {
    return /Material\.007/.test(m?.name || '');
  }

  function isPageBlockMat(m) {
    return /书页/.test(m?.name || '');
  }

  function isCaseShellMat(m) {
    return isCoverMat(m) || isSpineClothMat(m);
  }

  function isLetteringCoverMesh(obj) {
    return obj?.isMesh && /有字/.test(meshLabel(obj));
  }

  function isBlankCover(obj) {
    let cur = obj;
    while (cur) {
      if (/无字/.test(cur.name || '')) return true;
      cur = cur.parent;
    }
    return false;
  }

  /** Spine-square / back-logo host — 有字 mesh, not the blank 无字 shell. */
  function isFoilHostMesh(obj) {
    if (!obj?.isMesh || obj.visible === false) return false;
    if (isBlankCover(obj)) return false;
    if (isLetteringCoverMesh(obj)) return true;
    return meshMaterials(obj).some((m) => isCoverMat(m));
  }

  /** Hot-stamp black foil — sits in the recess, slight metal sheen */
  function blackFoilMat(envMap) {
    return new THREE.MeshPhysicalMaterial({
      color: 0x0a0a0a,
      metalness: 0.42,
      roughness: 0.55,
  ...
  /** Hot-stamp silver foil — NEXT spine squares + back logo */
  function silverFoilMat(envMap) {
    return new THREE.MeshPhysicalMaterial({
      color: 0x8a8e94,
      metalness: 0.86,
      roughness: 0.48,
  ...
  function foilStampMat(envMap) {
    return bookVolume() === 'next' ? silverFoilMat(envMap) : blackFoilMat(envMap);
  }

  /**
   * Fabric GLB stores a linen albedo jpeg as normalTexture.
   * Remap that weave into a silver-gray range so threads read without
   * crushing the case dark, or flattening it to a solid fill.
   */
  function cloneLinenMap(srcTex, uvRepeat, colorSpace) {
    const map = srcTex.clone();
    map.wrapS = THREE.RepeatWrapping;
    map.wrapT = THREE.RepeatWrapping;
    map.colorSpace = colorSpace;
    map.repeat.set(uvRepeat, uvRepeat);
    map.anisotropy = renderer?.capabilities?.getMaxAnisotropy?.() || 8;
    map.needsUpdate = true;
    return map;
  }

  function linenGrayAlbedo(srcTex, uvRepeat) {
    ...
    for (let i = 0; i < d.length; i += 4) {
      const g = (d[i] * 0.3 + d[i + 1] * 0.59 + d[i + 2] * 0.11) / 255;
      const t = (g - lo) / span;
      d[i] = dark[0] + (lit[0] - dark[0]) * t;
      d[i + 1] = dark[1] + (lit[1] - dark[1]) * t;
      d[i + 2] = dark[2] + (lit[2] - dark[2]) * t;
    }
  ...
  function fabricLinenMat(src, envMap, uvRepeat = 1) {
    const srcTex = src?.map || src?.normalMap || null;
    const map = srcTex ? linenGrayAlbedo(srcTex, uvRepeat) : null;
    const bump = srcTex ? cloneLinenMap(srcTex, uvRepeat, THREE.NoColorSpace) : null;
    const mat = new THREE.MeshPhysicalMaterial({
      color: 0xffffff,
      map,
      bumpMap: bump,
      ...
      clearcoat: 0,
      sheen: 0.32,
      sheenColor: new THREE.Color(0xa4a8ae),
      sheenRoughness: 0.78,
      side: THREE.FrontSide,
    });
    mat.name = src?.name || 'Material.007';
    return mat;
  }

  /** Silvered fore-edge — keep 书页.001 stack photo so page lines still read. */
  function silverPageEdgeMat(src, envMap) {
    ...
    mat.name = src?.name || '书页.001';
    return mat;
  }

  function applyNextCaseFinish(root, envMap) {
    const spineMat = { current: null };
    const coverMat = { current: null };
    const pageMatRef = { current: null };
    root.traverse((obj) => {
      if (!obj.isMesh || isBlankCover(obj)) return;
      const mats = meshMaterials(obj);
      const cloth = mats.find((m) => isSpineClothMat(m));
      const page = mats.find((m) => isPageBlockMat(m));
      if (cloth) {
        if (isLetteringCoverMesh(obj)) {
          if (!coverMat.current) coverMat.current = fabricLinenMat(cloth, envMap, 8);
          obj.material = coverMat.current;
        } else {
          if (!spineMat.current) spineMat.current = fabricLinenMat(cloth, envMap, 1);
          obj.material = spineMat.current;
        }
        return;
      }
      if (page) {
        if (!pageMatRef.current) pageMatRef.current = silverPageEdgeMat(page, envMap);
        obj.material = pageMatRef.current;
      }
    });
  }

  function extractMatchingTriangles(mesh, testWorld, requireAllVerts = false) {
    ...
    for (let t = 0; t < triCount; t += 1) {
      ...
      const i1 = idx ? idx.getX(t * 3 + 1) : t * 3 + 1;
      const i2 = idx ? idx.getX(t * 3 + 2) : t * 3 + 2;
      la.fromBufferAttribute(pos, i0);
      lb.fromBufferAttribute(pos, i1);
      lc.fromBufferAttribute(pos, i2);
      a.copy(la).applyMatrix4(mw);
      b.copy(lb).applyMatrix4(mw);
      c.copy(lc).applyMatrix4(mw);
      centroid.copy(a).add(b).add(c).multiplyScalar(1 / 3);
      nWorld.fromBufferAttribute(nrm, i0).applyMatrix3(nm).normalize();
      const pass = requireAllVerts
        ? testWorld(a, nWorld) && testWorld(b, nWorld) && testWorld(c, nWorld)
        : testWorld(centroid, nWorld);
      if (!pass) continue;
      na.fromBufferAttribute(nrm, i0);
      nb.fromBufferAttribute(nrm, i1);
      nc.fromBufferAttribute(nrm, i2);
      outPos.push(la.x, la.y, la.z, lb.x, lb.y, lb.z, lc.x, lc.y, lc.z);
      outNrm.push(na.x, na.y, na.z, nb.x, nb.y, nb.z, nc.x, nc.y, nc.z);
    }
  ...
  /**
   * Box in parent local space, +Z facing a world-space normal.
   * add() not attach() — cover node is already Ry(180).
   */
  function addWorldFacingBox(parent, worldCenter, worldNormal, width, height, depth, mat, name) {
    ...
    mesh.position.copy(worldCenter).applyMatrix4(inv);
    const nLocal = worldNormal.clone().transformDirection(inv).normalize();
    mesh.quaternion.setFromUnitVectors(new THREE.Vector3(0, 0, 1), nLocal);
    return mesh;
  }

  function stampSpineWell(coverMesh, well, mat, suffix) {
    const innerGeo = extractMatchingTriangles(coverMesh, (p) => (
      p.x > well.x0 && p.x < well.x1
      && Math.abs(p.y) <= well.y
      && p.z >= 0.1316 && p.z <= 0.13495
    ), true);
    if (innerGeo) {
      const inner = new THREE.Mesh(innerGeo, mat);
      inner.name = `foil-spine-${suffix}-inner`;
      inner.renderOrder = 9;
      coverMesh.add(inner);
    }
    addWorldFacingBox(
      coverMesh,
      new THREE.Vector3((well.x0 + well.x1) / 2, 0, 0.13435),
      new THREE.Vector3(0, 0, 1),
      well.x1 - well.x0 - 0.0016,
      well.y * 2 - 0.0028,
      0.00055,
      mat,
      `foil-spine-${suffix}`,
  ...
  function addFoilStamps(coverMesh, envMap) {
    coverMesh.updateMatrixWorld(true);
    const box = new THREE.Box3().setFromObject(coverMesh);
    const ymin = box.min.y;
    const foil = foilStampMat(envMap);

    const logoGeo = extractMatchingTriangles(coverMesh, (p, n) => (
      n.y < -0.8
      && p.y > ymin + 0.0002
      && p.y < ymin + 0.00055
      && Math.abs(p.x) < 0.02
    ...
    if (logoGeo) {
      const logo = new THREE.Mesh(logoGeo, foil);
      logo.name = 'foil-back-logo';
      logo.renderOrder = 9;
      coverMesh.add(logo);
    }

    /*
     * Top spine square is a hole in the outer +Z sheet. Behind it, boolean
     * leftover faces span the full spine width — painting those reads as a
     * horizontal black band. Only triangles whose *every* vertex sits inside
     * the square (and behind the outer face) are stamped; a slightly smaller
     * box fills the empty well without reaching the shoulders.
     * Pitch to the next square along the spine is 0.024 in model X.
     */
    const sq0 = { x0: -0.1020, x1: -0.0899, y: 0.00615 };
    stampSpineWell(coverMesh, sq0, foil, 'sq0');
    /* /next is volume 2/9 — fill the first two spine squares. */
    if (bookVolume() === 'next') {
      const pitch = 0.024;
      stampSpineWell(coverMesh, { x0: sq0.x0 + pitch, x1: sq0.x1 + pitch, y: sq0.y }, foil, 'sq1');
    }
  }

  async function buildBook(envMap) {
    const lentUrls = bookVolume() === 'next'
      ? [ap('assets/intro/next.png'), ap('assets/next-lenticular-b.png'), ap('assets/next-lenticular-c.png')]
      : ASSETS.lenticular;
    const lentTexs = await Promise.all(lentUrls.map((u) => loadTex(u)));
    const modelUrl = bookVolume() === 'next' ? ASSETS.bookModelFabric : ASSETS.bookModel;
    const gltf = await loadGltf(modelUrl);

    /* Fabric export ships a solid 无字 shell that fills the foil wells — hide it. */
    if (bookVolume() === 'next') {
      gltf.scene.traverse((obj) => {
        if (isBlankCover(obj)) obj.visible = false;
      });
    }
    ...
    gltf.scene.traverse((obj) => {
      ...
      for (const m of meshMaterials(obj)) {
        ...
        seenMats.add(m);
        if (bookVolume() !== 'next' && isCaseShellMat(m) && m.color) m.color.multiplyScalar(SHELL_ALBEDO);
        if (envMap && 'envMap' in m) {
          m.envMap = envMap;
          if (bookVolume() !== 'next' && isCaseShellMat(m)) m.envMapIntensity = SHELL_ENV;
          else if (m.envMapIntensity == null) m.envMapIntensity = 0.72;
        }
        m.needsUpdate = true;
      }
    });
    if (bookVolume() === 'next') applyNextCaseFinish(gltf.scene, envMap);

    gltf.scene.updateMatrixWorld(true);
    gltf.scene.traverse((obj) => {
      if (!obj.isMesh) return;
      if (isFoilHostMesh(obj)) addFoilStamps(obj, envMap);
    });
    ...
    shell.traverse((obj) => {
      ...
      if (isCoverStripeMesh(obj)) stripe = obj;
      if (isLetteringCoverMesh(obj)) coverMesh = obj;
      else if (!coverMesh && meshMaterials(obj).some((m) => isCoverMat(m))) coverMesh = obj;
    });

    const wellMesh = stripe || coverMesh;
    if (wellMesh) {
      if (stripe) stripe.visible = false;
      let lentW;
      let lentH;
      const pos = new THREE.Vector3();
      const measured = !stripe && coverMesh ? measureFrontCoverWell(coverMesh) : null;
      if (measured) {
        lentW = (measured.maxX - measured.minX) * 0.978;
        lentH = (measured.maxY - measured.minY) * 0.978;
        /* Sit on the cover face, filling the groove opening — not down in the well. */
        pos.set(
          (measured.minX + measured.maxX) * 0.5,
          (measured.minY + measured.maxY) * 0.5,
          measured.zOuter + LENTICULAR_Z,
        );
      } else {
        const box = new THREE.Box3().setFromObject(wellMesh);
        const size = box.getSize(new THREE.Vector3());
        const center = box.getCenter(new THREE.Vector3());
        const face = Math.min(size.x, size.y);
        lentW = stripe ? Math.max(0.01, size.x) : face * 0.196;
        lentH = stripe ? Math.max(0.01, size.y) : face * 0.196;
        pos.set(center.x, center.y, box.max.z + LENTICULAR_Z);
      }
  ...
  async function openBook() {
    /* Reader lives on /emax/pages — no in-page 3D→flat handoff */
    if (opening || holdCompleted) return;
    if (bookVolume() === 'next') return;
    opening = true;
    location.assign(ap('emax/pages/'));
  }
  ...
  function updateHoldRing(dt) {
    const mat = holdRingMesh?.material;
    ...
    const holding =
      ...
      !holdCompleted &&
      mode === 'closed';
    const stillMs = holding ? performance.now() - holdStart : 0;
    const armed = holding && stillMs >= HOLD_ARM_MS;
    if (pointerMoved && !holdCompleted) {
      holdOpacity = 0;
      holdProgress = 0;
    } else if (armed) {
      holdProgress = Math.min(1, (stillMs - HOLD_ARM_MS) / HOLD_MODEL_MS);
      holdOpacity = Math.min(1, holdOpacity + dt / 0.12);
      if (holdProgress >= 1) openModel();
    } else if (!holdCompleted) {
      holdOpacity = Math.max(0, holdOpacity - dt / 0.16);
      if (holdOpacity <= 0.001) holdProgress = 0;
    }
    mat.uniforms.uProgress.value = holdProgress;
    mat.uniforms.uOpacity.value = holdOpacity;
    holdRingMesh.visible = holdOpacity > 0.01;
  }
  ...
  /** After homepage intro — refit once the overlay is gone. */
  function startEntranceReveal() {
    if (!canvas || !active) return;
    requestAnimationFrame(() => {
      if (active && canvas) fitBookToView();
    });
    if (!canvas.classList.contains('is-loaded')) canvas.classList.add('is-loaded');
  }
  ...
  function fitBookToView() {
    ...
    fitCamera();
    const { w, h } = getDisplaySize();
    const slot = canvas?.closest('.detail-visual--book');
    const slotR = slot?.getBoundingClientRect();
    const slotW = Math.max(1, slotR?.width || w);
    const slotH = Math.max(1, slotR?.height || h);
    const coverV = Math.max(h / slotH, 1);
    const fovSlot = CLOSED_FOV;
    ...
    const slotVFov = THREE.MathUtils.degToRad(fovSlot);
    const slotHFov = 2 * Math.atan(Math.tan(slotVFov * 0.5) * (slotW / slotH));
    const vFov = THREE.MathUtils.degToRad(camera.fov);
    const hFov = 2 * Math.atan(Math.tan(vFov * 0.5) * camera.aspect);
    const dist = Math.max(
      radius / Math.sin(slotVFov * 0.5),
      radius / Math.sin(slotHFov * 0.5),
      (radius + Math.abs(CLOSED_CAM_Y) * radius) / Math.sin(vFov * 0.5),
      (radius + Math.abs(CLOSED_CAM_X) * radius) / Math.sin(hFov * 0.5),
    );
  ...
  function onPointerDown(e) {
    if (!active || opening || holdCompleted || mode !== 'closed') return;
    if (e.button != null && e.button !== 0) return;
    if (document.body.classList.contains('is-emax-intro')) return;
  ...
  function isPointerOnLenticular(clientX, clientY) {
    if (!lenticularMesh || !camera || !canvas) return false;
    ...
    return hoverRay.intersectObject(lenticularMesh, false).length > 0;
  }

  function updateLenticularCursor(e) {
    if (!canvas) return;
    if (!active || opening || holdCompleted || mode !== 'closed') {
      canvas.classList.remove('is-over-lenticular');
      return;
    }
    if (dragging && pointerMoved) {
      canvas.classList.remove('is-over-lenticular');
      return;
    }
    canvas.classList.toggle('is-over-lenticular', isPointerOnLenticular(e.clientX, e.clientY));
  }

  function onPointerMove(e) {
    updateLenticularCursor(e);
    if (!dragging || mode !== 'closed') return;

    const dx = e.clientX - pointerDownX;
    const dy = e.clientY - pointerDownY;
    if (!pointerMoved && Math.hypot(dx, dy) > DRAG_THRESHOLD) {
      /* Vertical swipe scrolls the page; only rotate on a sideways drag. */
      ...
      holdProgress = 0;
      holdOpacity = 0;
      if (holdRingMesh) holdRingMesh.visible = false;
  ...
  function onPointerUp(e) {
    ...
    const heldMs = holdArmed ? performance.now() - holdStart : 0;
    dragging = false;
    holdArmed = false;
    canvas.classList.remove('is-dragging');
    if (e.type !== 'pointercancel') updateLenticularCursor(e);
    if (pointerCaptured) {
    ...
    if (e.type === 'pointercancel' || moved || opening || mode !== 'closed') {
      if (moved || e.type === 'pointercancel') lastTapTime = 0;
      return;
    }
    if (heldMs >= HOLD_TAP_CANCEL_MS) {
      lastTapTime = 0;
      return;
    }
  ...
  function animate() {
    ...
    if (mode === 'closed' && !opening) {
      ...
      updateHoldRing(dt);
    } else if (mode === 'closed' && holdCompleted) {
      updateHoldRing(dt);
  ...
  async function init(containerId) {
    ...
    try {
      ...
      if (!canvas.__bookCtxBound) {
        canvas.__bookCtxBound = true;
      ...
      scene.add(front);
      const spineLight = new THREE.DirectionalLight(0xe8ecf2, 0.42);
      spineLight.position.set(-8, 1.6, 1.2);
      scene.add(spineLight);
      scene.add(new THREE.AmbientLight(0xb4bcc6, 0.22));
      scene.add(new THREE.HemisphereLight(0xe0e4ee, 0x2a2e34, 0.28));
  ...
  function destroy() {
    active = false;
    cancelAnimationFrame(rafId);
    canvas?.classList.remove('is-pixel-revealing', 'is-over-lenticular', 'is-dragging');
    reader?.dispose();
    reader = null;
    ...
    lenticularMesh = null;
    if (holdRingMesh) {
      holdRingMesh.geometry.dispose();
      holdRingMesh.material.dispose();
      holdRingMesh = null;
    }
    holdArmed = false;
    holdCompleted = false;
