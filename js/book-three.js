...
const BookViewer = (() => {
  ...
  function isCaseShellMat(m) {
    const n = m?.name || '';
    return /白色封面/.test(n) || /Material\.007/.test(n);
  }
  ...
  function addBlackFoilStamps(coverMesh, envMap) {
    coverMesh.updateMatrixWorld(true);
    const box = new THREE.Box3().setFromObject(coverMesh);
    const ymin = box.min.y;
    const foil = blackFoilMat(envMap);
    ...
    if (innerGeo) {
      const inner = new THREE.Mesh(innerGeo, foil);
      inner.name = 'foil-spine-sq0-inner';
      inner.renderOrder = 9;
      coverMesh.add(inner);
    }
  ...
  async function buildBook(envMap) {
    const lentTexs = await Promise.all(ASSETS.lenticular.map((u) => loadTex(u)));
    ...
    gltf.scene.traverse((obj) => {
      ...
      for (const m of meshMaterials(obj)) {
        ...
        seenMats.add(m);
        if (isCaseShellMat(m) && m.color) m.color.multiplyScalar(SHELL_ALBEDO);
    ...
    gltf.scene.traverse((obj) => {
      if (!obj.isMesh) return;
      if (meshMaterials(obj).some((m) => /白色封面/.test(m?.name || ''))) {
        addBlackFoilStamps(obj, envMap);
      }
    });
    ...
    shell.traverse((obj) => {
      ...
      if (meshMaterials(obj).some((m) => /白色封面/.test(m?.name || ''))) coverMesh = obj;
    });
