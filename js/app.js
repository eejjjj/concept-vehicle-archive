...
function bootCurrentPage() {
  ...
  if (page === 'emax-pages') initEmaxPages();
  else if (page === 'emax-model') initEmaxModel();
...
function emaxPagesHome() { return sitePath('emax/pages/'); }
function emaxModelHome() { return sitePath('emax/model/'); }
function nextHome() { return sitePath('next/'); }
function infoHome() { return sitePath('info/'); }

/** Header 导航中不可点击的项（页面仍可通过直接 URL 访问） */
const DISABLED_NAV_IDS = new Set();
...
function initArchiveEffects() {
  ...
  function bootEmaxPages() {
    const canvas = document.getElementById('book-canvas');
    const start = () => {
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          if (!window.EmaxPages) return;
          EmaxPages.init('book-canvas');
        });
      });
    };
    const fail = () => {
      if (!canvas || window.EmaxPages) return;
    ...
    if (window.EmaxPages) start();
    else {
      document.addEventListener('emax-pages:ready', start, { once: true });
      setTimeout(fail, 10000);
    }
  }

  function initEmaxPages() {
    document.body.classList.add('is-book-reading', 'is-page-enter-active');
    if (window.matchMedia(window.CVA_MQ?.phonePortrait || '(max-width: 900px) and (orientation: portrait)').matches) {
    ...
    document.title = 'EMAX Pages — Concept Vehicle Archive';
    revealDetailBody();
    initArchiveEffects();
    Motion.revealChrome();
  ...
  function bootEmaxModel() {
    const canvas = document.getElementById('model-canvas');
    const start = () => {
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          if (!window.EmaxModel) return;
          EmaxModel.init('model-canvas');
        });
      });
    };
    const fail = () => {
      if (!canvas || window.EmaxModel) return;
      canvas.classList.add('is-error', 'is-loaded');
      const tip = document.createElement('p');
      tip.className = 'detail-visual__fallback';
      tip.textContent = '3D model failed to load. Hard-refresh or run: bash preview.sh';
      canvas.parentElement?.appendChild(tip);
    };
    if (window.EmaxModel) start();
    else {
      document.addEventListener('emax-model:ready', start, { once: true });
      setTimeout(fail, 12000);
    }
  }

  function initEmaxModel() {
    document.body.classList.add('is-book-reading', 'is-page-enter-active');
    if (window.matchMedia(window.CVA_MQ?.phonePortrait || '(max-width: 900px) and (orientation: portrait)').matches) {
      const hint = document.getElementById('reader-rotate-hint');
      if (hint) hint.hidden = false;
    ...
    const bar = document.getElementById('book-reader-bar');
    if (bar) bar.hidden = false;
    document.title = 'EMAX Model — Concept Vehicle Archive';
    revealDetailBody();
    initArchiveEffects();
    Motion.revealChrome();
    Motion.bootPageEnter(true);
  ...
  if (fromHomeIntro) {
    ...
    revealDetailBody();
    initArchiveEffects();
    if (document.getElementById('book-canvas')) bootBookViewer();
...
window.addEventListener('pageshow', (e) => {
  ...
  if (document.body.dataset.page !== 'detail' || !document.getElementById('book-canvas')) return;
...
window.addEventListener('pageshow', (e) => {
  if (!document.body.classList.contains('is-intro') || !window.IntroGrid) return;
  if (!e.persisted) return;
  IntroGrid.destroy();
  requestAnimationFrame(() => IntroGrid.init('intro-canvas'));
});
