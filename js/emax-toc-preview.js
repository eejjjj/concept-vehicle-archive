...
(() => {
  ...
  const MAX_BLOCK = 48;
  const REVEAL_MS = 780;
  const MOBILE_MQ = window.CVA_MQ?.phone
    || '(max-width: 900px), (pointer: coarse) and (hover: none) and (max-height: 600px), (max-width: 1100px) and (max-height: 600px) and (orientation: landscape)';
  ...
  function resolveSrc(row) {
    ...
    const file = page1 === 9 ? '260623_emax9.png' : `260623_emax${page1}.jpg`;
  ...
  function bindRow(row) {
    ...
    const flashPagesHint = () => {
      const btn = document.getElementById('emax-pages-btn');
      if (!btn) return;
    ...
    /* Mobile: tap to toggle · one open at a time · tap outside closes
       Desktop: click chapter text flashes OPEN (no TOC link) */
    ...
    const pagesBtn = document.getElementById('emax-pages-btn');
    if (pagesBtn && !pagesBtn.dataset.hintFlashBound) {
      pagesBtn.dataset.hintFlashBound = '1';
      pagesBtn.addEventListener('animationend', (ev) => {
        if (ev.animationName === 'emax-pages-hint') {
          pagesBtn.classList.remove('is-hint-flash');
        }
      });
    }
