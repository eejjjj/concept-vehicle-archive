...
import { BookReader, PAGE_W as READER_PAGE_W } from './book-reader.js?v=flip07';
import { SPREAD_COUNT, spreadPages } from './book-pages.js?v=p3';
...
const BookViewer = (() => {
  ...
  function applyPlanarUVs(geometry, mode = 'cover') {
    ...
    for (let i = 0; i < pos.count; i += 1) {
      ...
      if (mode === 'spine') {
        ...
        /* 1mm strip (X) × book height (Z): texture is tall — U short, V long. */
        u = (x - bb.min.x) / sx;
        v = (z - bb.min.z) / sz;
      } else if (mode === 'block') {
        if (sx >= sy && sx >= sz) {
   ...
   * which shows as hairline seams. Rebuild a clean wrap with 9 holes only.
   */
  function replaceSpineAsCleanHoledWrap(spineMesh) {
  ...
  /** Matte silver case — satin metal, not chalk grey or chrome. */
  function matteSilverMat(envMap, map, normalMap, opts = {}) {
    const mat = new THREE.MeshStandardMaterial({
      color: opts.color ?? 0x8f949c,
      ...
      envMap,
      envMapIntensity: opts.envMapIntensity ?? 0.58,
  ...
  /** 书芯侧边：book_side.png 横纹沿纸厚方向（BoxGeometry 默认 UV 已对齐） */
  function sidePaperMat(sideTex, layout = 'fore') {
    const map = cloneMap(sideTex, THREE.SRGBColorSpace);
    const bump = cloneMap(sideTex, THREE.NoColorSpace);
    if (layout === 'tail') {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float tBlend = clamp(uBlend, 0.0, 1.0);
          /* Rest pose: A (1/9) + B (vehicle) both half-transparent. Ends: A only / C only. */
          float u0 = clamp(tBlend * 2.0, 0.0, 1.0);
          float u1 = clamp((tBlend - 0.5) * 2.0, 0.0, 1.0);
          float wA = mix(1.0, 0.5, u0) * (1.0 - u1);
          float wB = mix(0.0, 0.5, u0) * (1.0 - u1);
          float wC = u1;
  ...
  /** Horizontal drag from the rest pose: both directions leave the middle frame. */
  function updateLenticularBlend() {
    const mat = lenticularMesh?.material;
    if (!mat?.uniforms?.uBlend) return;
    const span = 0.48;
    mat.uniforms.uBlend.value = THREE.MathUtils.clamp(
      (rotY - DEFAULT_ROT_Y) / span + 0.5,
      0,
      1,
    );
  }
  ...
  async function buildBook(envMap) {
    ...
    const [spineTex, clothTex, , , backTex, coverTex, ...lentTexs] =
      texPack;
    const gltf = await loadGltf(ASSETS.bookModel);

    const SILVER = 0x8f949c;
    ...
    replaceSpineAsCleanHoledWrap(spineMesh);
    /* Spine-side: tuck hinge under the wrap. Cover-side: overlap the board to hide the hard join. */
    nudgeMinX(frontHinge, 0.00022);
    nudgeMinX(backHinge, 0.00022);
    nudgeMaxX(frontHinge, 0.00055);
    nudgeMaxX(backHinge, 0.00055);
    nudgeMinX(frontMesh, 0.00028);
    nudgeMinX(backMesh, 0.00028);
    if (recessBackMesh) {
    ...
    const clothMap = cloneMap(clothTex, THREE.SRGBColorSpace);
    const clothBump = cloneMap(clothTex, THREE.NoColorSpace);
  ...
  async function init(containerId) {
    ...
    try {
      ...
      scene.add(new THREE.AmbientLight(0xa8b0bc, 0.48));
      scene.add(new THREE.HemisphereLight(0xd8dde8, 0x202428, 0.32));
