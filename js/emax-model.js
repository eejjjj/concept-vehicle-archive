...
const EmaxModel = (() => {
  ...
  /** Phone landscape: keep the long side-profile inside the width at max zoom. */
  function zoomMax() {
    if (!(isPhone() && isLandscapeLayout())) return ZOOM_MAX;
    const { w, h } = getDisplaySize();
    const aspect = w / Math.max(1, h);
    const vFov = THREE.MathUtils.degToRad(CLOSED_FOV);
    const hFov = 2 * Math.atan(Math.tan(vFov * 0.5) * aspect);
    const limit = Math.sin(hFov * 0.5) / Math.sin(vFov * 0.5);
    return Math.max(LANDSCAPE_ZOOM, Math.min(ZOOM_MAX, limit * 0.9));
  }
  ...
  function fitCarToView() {
    ...
    if (viewZoom > zoomMax()) viewZoom = zoomMax();
