...
`https://<your-username>.github.io/concept-vehicle-archive/index/`
...
bash preview.sh
# open http://localhost:8765/index/
```
...
```
concept-vehicle-archive/
├── index.html          → redirects to /index/
├── index/              → L1 intro (wind wall)
...
| Level | Page | URL |
|-------|------|-----|
| L1 · Intro | Wind wall entrance | `/index/` |
...
- `/index/` — intro only; click anywhere to enter the archive
- `/catalog/` — the catalog/directory page (module grid)
- Logo on any page → `/index/` (back to wind wall)
