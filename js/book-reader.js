...
export class BookReader {
  ...
  _progressFromPointer(layoutX, layoutY, drag, clientX = null) {
    const L = this._screenLayout();
    const rect = drag.dir === 'fwd' ? L.right : L.left;
    const mobile = isMobileReaderRotated();
    const span = Math.max(48, (mobile ? Math.max(rect.w, rect.h) : rect.w) * 0.88);
    let progress;
    if (mobile && clientX != null && drag.startClientX != null) {
      /* Screen space: left swipe → next, right swipe → prev */
      if (drag.dir === 'fwd') {
        progress = THREE.MathUtils.clamp((drag.startClientX - clientX) / span, 0, 1);
      } else {
        progress = THREE.MathUtils.clamp((clientX - drag.startClientX) / span, 0, 1);
      }
    ...
    const cy = (layoutY - rect.y) / Math.max(1, rect.h);
    const pivotY = THREE.MathUtils.clamp((0.5 - cy) * PAGE_H, -PAGE_H * 0.45, PAGE_H * 0.45);
  ...
  _beginDrag(dir, clientX, clientY, pointerId) {
    ...
    const cy = (p.y - rect.y) / Math.max(1, rect.h);
    const pivotY = THREE.MathUtils.clamp((0.5 - cy) * PAGE_H, -PAGE_H * 0.45, PAGE_H * 0.45);
    this._drag = {
      dir,
  ...
  _onPointerDown(e) {
    ...
    /* Mobile rotated: arm swipe — left = next, right = prev */
    if (isMobileReaderRotated()) {
      const p = clientToCanvasLayout(this.canvas, e.clientX, e.clientY);
      const L = this._screenLayout();
      const inSpread =
        p.x >= L.left.x &&
        p.x <= L.right.x + L.right.w &&
        p.y >= Math.min(L.left.y, L.right.y) &&
        p.y <= Math.max(L.left.y + L.left.h, L.right.y + L.right.h);
      if (!inSpread) return;
      e.preventDefault();
      this._swipeArm = {
  ...
  _onPointerMove(e) {
    ...
    if (this._swipeArm && e.pointerId === this._swipeArm.pointerId && !this._drag) {
      ...
      /* Prefer horizontal screen swipe */
      ...
      if (!this._beginDrag(dir, startX, startY, e.pointerId)) return;
      /* Apply current pointer as first progress sample */
    }
