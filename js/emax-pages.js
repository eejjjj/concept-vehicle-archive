/**
 * /emax/pages · standalone flat reader (no 3D hardcover handoff)
 */
import * as THREE from 'three';
import { RGBELoader } from 'three/addons/loaders/RGBELoader.js';
import { RoomEnvironment } from 'three/addons/environments/RoomEnvironment.js';
import { BookReader, HOLD_DELAY, HOLD_INTERVAL, isPortraitPanMode } from './book-reader.js?v=flip72';
import { spreadPages, SPREAD_COUNT } from './book-pages.js?v=p13';
...
function emaxModelUrl() {
  return ap('emax/model/');
}

const PORTRAIT_MQ = window.CVA_MQ?.phonePortrait || '(max-width: 900px) and (orientation: portrait), (pointer: coarse) and (hover: none) and (orientation: portrait) and (max-width: 500px)';
...
const EmaxPages = (() => {
  ...
  function bindReaderNav() {
    ...
    /* iOS <a> ignores :active — press class so chrome flashes purple. */
    bindBarLink(back, emaxUrl);
    bindBarLink(document.getElementById('pages-model-open'), emaxModelUrl);
  }
  ...
  function concealMobileBrowserChrome() {
    const on = window.matchMedia(PHONE_LANDSCAPE_MQ).matches;
    const html = document.documentElement;
    const body = document.body;
    html.classList.toggle('is-reader-landscape', on);
    if (!on) {
  ...
  function onViewportChrome() {
    if (document.documentElement.classList.contains('is-reader-landscape')) {
      applyReaderViewportHeight();
    }
    onResize();
  }
  ...
  async function init(canvasId = 'book-canvas') {
    ...
    try {
      ...
      console.error('[EmaxPages] init failed', err);
      destroy();
...
window.EmaxPages = EmaxPages;
document.dispatchEvent(new CustomEvent('emax-pages:ready'));
