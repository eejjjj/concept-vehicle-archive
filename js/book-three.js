...
const BookViewer = (() => {
  ...
  async function buildBook(envMap) {
    ...
    const well = stripe || coverMesh;
    if (well) {
      if (stripe) stripe.visible = false;
      const box = new THREE.Box3().setFromObject(well);
      const size = box.getSize(new THREE.Vector3());
      const center = box.getCenter(new THREE.Vector3());
      /* v01 条纹 was ~19.6% of the cover face; v02 bakes the well into the cover. */
      const face = Math.min(size.x, size.y);
      const lentW = stripe ? Math.max(0.01, size.x) : face * 0.196;
      const lentH = stripe ? Math.max(0.01, size.y) : face * 0.196;
      const lenticular = new THREE.Mesh(
        new THREE.PlaneGeometry(lentW, lentH),
        lenticularMat(lentTexs),
      );
      const pos = new THREE.Vector3(center.x, center.y, box.max.z + LENTICULAR_Z);
