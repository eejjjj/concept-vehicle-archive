...
const BookInside = (() => {
  ...
  function applyReaderViewportHeight() {
    ...
    let h = Math.round(vv || inner || shortEdge || 0);
    /* Chrome collapsed: snap to the device short edge (portrait page size). */
    if (shortEdge && h >= shortEdge - 8) h = shortEdge;
    document.documentElement.style.setProperty('--reader-vvh', `${h}px`);
  }

  function concealMobileBrowserChrome() {
    const on = window.matchMedia('(max-width: 900px) and (orientation: landscape)').matches;
