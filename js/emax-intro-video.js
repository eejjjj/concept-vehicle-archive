...
const EmaxIntroVideo = (() => {
  ...
  function cleanup() {
    ...
    if (root) {
      ...
      root.classList.remove('is-pixelating', 'is-ready');
      root.style.opacity = '';
    }
    ...
    return new Promise((resolve) => {
      ...
      const src =
        typeof window.assetPath === 'function'
          ? window.assetPath('videos/emaxintro.mp4')
          : '../videos/emaxintro.mp4';
      const sourceEl = video.querySelector('source');
      if (sourceEl) {
        sourceEl.type = 'video/mp4';
        if (sourceEl.getAttribute('src') !== src) sourceEl.setAttribute('src', src);
